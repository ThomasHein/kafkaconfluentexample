...
/**
 * See Tutorial at for producer https://dzone.com/articles/kafka-avro-serialization-and-the-schema-registry for streams https://aseigneurin.github
 * .io/2018/08/06/kafka-tutorial-7-kafka-streams-serdes-and-avro.html Gradle Plugin Generation http://cloudurable.com/blog/avro/index.html
 * <p>
 * Please the sChema tab in confluent gui topic -> sChema http://192.168.99.100:9021/clusters/Ze_NOufZQE-YVgrudC7eTw/management/topics/order-avro-topic/schema/value
 * <p>
 * { "name": "OrderAvro", "type": "record", "namespace": "de.inmediasp.de.soft.demo.kafka", "fields": [ {"name": "id", "type": "int"}, {"name": "amount", "type": "double"}, {"name": "personId",
 * "type": "int"}, {"name": "car", "type": "string"} ] }
 */
public class OrderAvroWriter5 {
    ...
    private void workingExample() {
        ...
        try {
            while (true) {
                sendMessageCount++;
                OrderAvro o = OrderAvro.newBuilder().setCar("Trabbi Avor").setId(new Double(Math.random() * 1000).intValue()).setAmount(Math.random() * 1000)
                        .setPersonId(new Double(Math.random() * 10).intValue()).build();
    ...
    /**
     * Be carefully this is not working because the shema is not known to the registry There is no alignment with Kafka if the registry is aligned? If you would register this schema to the registry
    ...
    private void notWorkingExampleBecauseOfWrongSchema() {
        ...
        try {
            while (true) {
                sendMessageCount++;
                OrderNotWorkingAvro o = OrderNotWorkingAvro.newBuilder().setId(222).setName("Test").build();
