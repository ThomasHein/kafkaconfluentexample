...
import static com.example.demokafka.KafkaConfiguration.getOrderProperties;
import static com.example.demokafka.KafkaConfiguration.getPersonProperties;
...
/**
 * Play a little with the writer turn it of and on and set have happend with the consumer, when does it provide an output. What meesages does the consumer take after a restart, all or just the last
 * once?
 */
public class ReadingOrderFromTopic1 {
    /**
     * No nice code only once processing is not guaranteed
     */
    public static void readOrderPerPollOldWay1() {

        KafkaConsumer<String, String> consumer = new KafkaConsumer<String, String>(getOrderProperties());
    ...
    /**
     * Differences Consumer And Stream API https://stackoverflow.com/questions/44014975/kafka-consumer-api-vs-streams-api Big Big Benefit Quaranteed only once processing Easier selection of
     * information
     */
    ...
    public static void readPersonPerStream() {
        ...
        builder.stream(personinput, Consumed.with(Serdes.String(), PersonSerde.getPersonSerde())).groupByKey().count().toStream()
