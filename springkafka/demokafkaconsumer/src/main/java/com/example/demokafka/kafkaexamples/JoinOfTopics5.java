...
import static com.example.demokafka.KafkaConfiguration.getPersonJoinProperties;
...
public class JoinOfTopics5 {
    ...
    public static void readPersonAndOrder() {
        ...
        persons.join(orders, (personValue, orderValue) -> {
            ...
            return null;
        }, JoinWindows.of(Duration.ofNanos(30)), Joined.with(Serdes.String(), /*key*/
                PersonSerde.getPersonSerde(),/*left*/
                OrderSerde.getOrderSerde() /*right*/)).filter((key, value) -> value != null).foreach((key, value) -> {
            if (value == null) {
